...
def handle_gemm(network, layer_idx, config, model_calib):
    """
    ...
    if 'weight_bits' in config[layer.name]:
        ...
        model_calib[pre_in_tensor.name] = (tracked_max_input - tracked_min_input)/255
        model_calib[in_tensor.name] = (tracked_max_input - tracked_min_input)/255
        model_calib[next_in_tensor.name] = (tracked_max_input - tracked_min_input)/255
    ...
    if 'output_bits' in config[layer.name]:
        ...
        model_calib[next_out_tensor.name] = (tracked_max_output - tracked_min_output)/255
...
def build_engine(model_file, config=None, extra_layer_bits=32, strict_datatype=False, calib=None):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(common.EXPLICIT_BATCH) as network, \
        ...
        if calib is not None:
            ...
            name_list = [network.get_layer(i).name for i in range(network.num_layers)]
            # pdb.set_trace()
            ...
            for i in range(network.num_layers):
                ...
                if layer.name[0:4] == "Gemm":
                    handle_gemm(network, i, config, model_calib)
                    continue
...
class ModelSpeedupTensorRT(BaseModelSpeedup):
    ...
    def compress(self):
        ...
        assert self.input_shape is not None
         ...
         # Convert pytorch model to onnx model and store model_calib in self.calibration_cache
        _, self.onnx_config =  fonnx.get_onnx_calib(self.model, self.config, input_shape=self.input_shape,
            onnx_path=self.onnx_path, calib_path= self.calibration_cache, input_names=self.input_names, output_names=self.output_names)
        ...
        if self.calib_data_loader is not None:
            ...
            context = self._tensorrt_build_withoutcalib(self.onnx_path) # QAT
